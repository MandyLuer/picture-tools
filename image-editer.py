...
class WordImageExtractorApp:
    ...
    def create_extract_tab(self):
        ...
        ttk.Entry(file_frame, textvariable=self.word_file_path, width=50).grid(
            row=0, column=1, padx=5
        )
        ttk.Button(file_frame, text="浏览...", command=self.browse_word_file).grid(
            row=0, column=2
        )
        ...
        ttk.Label(output_frame, text="输出文件夹路径:").grid(
            row=0, column=0, sticky=tk.W
        )
        ttk.Entry(output_frame, textvariable=self.image_folder_path, width=50).grid(
            row=0, column=1, padx=5
        )
        ttk.Button(
            output_frame, text="浏览...", command=self.browse_output_folder
        ).grid(row=0, column=2)
        ...
        ttk.Button(button_frame, text="提取图片", command=self.extract_images).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            button_frame, text="打开图片文件夹", command=self.open_image_folder
    ...
    def create_rename_tab(self):
        ...
        ttk.Entry(folder_frame, textvariable=self.image_folder_path, width=50).grid(
            row=0, column=1, padx=5
        )
        ttk.Button(folder_frame, text="浏览...", command=self.browse_image_folder).grid(
            row=0, column=2
        )

        ttk.Button(folder_frame, text="加载图片", command=self.load_image_files).grid(
            row=1, column=2, pady=5, sticky=tk.E
        )
        ...
        ttk.Button(nav_frame, text="上一张", command=self.previous_image).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(nav_frame, text="下一张", command=self.next_image).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(nav_frame, text="保存重命名", command=self.save_rename).pack(
            side=tk.LEFT, padx=2
        )
        # # 重置按钮移至这里
        ...
        ttk.Button(
            row1_frame,
            text="左转90°",
            command=lambda: self.zoomable_image.rotate_image(-90),
        ...
        # 翻转按钮
        ttk.Button(
            row1_frame, text="水平翻转", command=self.zoomable_image.flip_horizontal
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            row1_frame, text="垂直翻转", command=self.zoomable_image.flip_vertical
        ).pack(side=tk.LEFT, padx=2)
        ...
        # 裁剪按钮
        ttk.Button(
            row2_frame, text="开始裁剪", command=self.zoomable_image.start_cropping
        ).pack(side=tk.LEFT, padx=2)
        ttk.Button(
            row2_frame, text="确认裁剪", command=self.zoomable_image.crop_image
        ).pack(side=tk.LEFT, padx=2)
        # 添加取消裁剪按钮
        ttk.Button(
            row2_frame, text="取消裁剪", command=self.zoomable_image.cancel_cropping
        ).pack(side=tk.LEFT, padx=2)

        # 压缩按钮
        ttk.Button(
            row2_frame, text="压缩图片", command=self.compress_current_image
        ).pack(side=tk.LEFT, padx=2)

        # 重置视图
        ttk.Button(
            row2_frame, text="重置视图", command=self.zoomable_image.reset_view
        ).pack(side=tk.LEFT, padx=2)
    ...
    def extract_images(self):
        ...
        try:
            ...
            if valid_images:
                messagebox.showinfo(
                    "完成", f"成功提取 {len(valid_images)} 张图片到 {output_folder}"
                )
                # 自动切换到重命名标签页
    ...
    def get_image_order_from_docx(self, docx_path):
        ...
        try:
            with zipfile.ZipFile(docx_path) as z:
                ...
                for pict in picts:
                    embeddings = pict.findall(
                        ".//v:shape/v:imagedata", {"v": "urn:schemas-microsoft-com:vml"}
                    )
                    for embedding in embeddings:
                        rel_id = embedding.get(
            ...
            try:
                ...
                for ref in image_refs:
                    # 在rels文件中查找实际的图片文件名
                    match = re.search(
                        f'Id="{ref}"[^>]*Target="media/([^"]+)"', rels_content
                    )
                    if match:
                ...
                with zipfile.ZipFile(docx_path) as z:
                    ...
                    media_files.sort(
                        key=lambda x: (
                            ...
                            if re.findall(r"\d+", os.path.basename(x))
                            else 0
                        )
                    )
